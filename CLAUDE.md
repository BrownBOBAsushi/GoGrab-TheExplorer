# CLAUDE.md — GrabExplore Hackathon Build Guide
...
|---|---|---|
| 1 | Slide 3 said "Claude API" but backend used GPT-4o — contradiction | Standardised to Claude claude-sonnet-4-20250514 throughout |
...
- **AI Trip Planning chatbot** — bottom left corner, appears on load, powered by Claude claude-sonnet-4-20250514
- Tourist types their preference: "I love street food"
- Claude API returns 3 nearest food POI recommendations based on user location
...
**You already have a working prototype from Claude design. Use it as your base. Do NOT start from zero.**
...
BACKEND (Node.js proxy — runs locally on your laptop):
  ...
  Routes:
    ...
    POST /api/chat         → proxies Anthropic Claude API (trip planning agent)
...
// Proxy: Claude API (trip planning agent)
...
> ⚠️ Claude returns POI names as strings. If the name doesn't exactly match your POI array,
...
3. AI chatbot appears on load, bottom left — powered by Claude
...
"Claude finds the 3 best matching spots — hyperlocal places built from real GrabMaps data."
...
Hour 2  (10–11am)  → Build chatbot panel + wire /api/chat (Claude).
...
> 🧳 Tourist mode: Claude AI asks what you love → 3 hidden spots → spin → voucher → GrabCar
> 🗺️ Explorer mode: Fog of war → discover like an RPG
> [Live demo here]

**Slide 3 — Stack + Roadmap**
> GrabMaps APIs: Nearby · Directions · Vector Map Tiles
> Anthropic: Claude claude-sonnet-4-20250514 — trip planning agent
...
| Everything broken | Open Claude design prototype as fallback, demo visually |
...
- Claude (Anthropic) + GrabMaps — consistent across code, slides, and pitch
